      ...
      def rules_strict
        {
          ...
          /( |\t)+/ => ' ',
          # Запятые после «а» и «но». Если уже есть — не ставим. Исключаем случай с частицей "ну": ну а также
          /(?=[^ну])([а-яA-я0-9])\s(а|но)\s/ => '\1, \2 ',
        }
      end
      ...
      def rules_main
        {
          ...
          /\b(кое|кой)[\s-](как|кого|какой)\b/i => "<nobr>\\1#{@minus}\\2</nobr>",
...
### to od          /(кто|что|где|когда|почему|зачем|кем|чем|как|чего)(\s|-|–|—|&nbsp;)+(либо|нибудь|то)([^:]|$)/i => "<nobr>\\1#{ndash}\\3</nobr>\\4",
          ...
          # /(все)(\s|-|–|—|&nbsp;)+(таки)/i => "<nobr>\\1#{@mdash}\\3</nobr>",
          ...
          # Неразрывный пробел после инициалов: В.И. Вернадский
          /([А-ЯA-Z]\.)([А-ЯA-Z]\.)\p{Zs}?([А-ЯA-Z][а-яa-z]+)/m => '\1\2&nbsp;\3',
          # Неразрывный пробелы внутри инициалов, если присутствует пробел: В. И. Вернадский
          /([А-ЯA-Z]\.)\s{1}([А-ЯA-Z]\.)\p{Zs}?([А-ЯA-Z][а-яa-z]+)/m => '\1&nbsp;\2&nbsp;\3',
